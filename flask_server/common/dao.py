...
from flask_server.common.config import DATABASE_NAME, USER, PASSWORD, DATABASE_PORT
...
class Dao:
    ...
    def add_locker(self, locker_id, location, status="available"):
        with self.conn.cursor() as cursor:
            cursor.execute("""INSERT INTO locker (id, location, status) VALUES (%s, %s, %s)""", (locker_id, location, status))
        self.conn.commit()
    ...
    def add_session(self, session_id, locker_id, palm_hash="empty", status="active"):
        try:
            with self.conn.cursor() as cursor:
                ...
                cursor.execute("""UPDATE locker SET status = 'occupied' WHERE id = %s AND status = 'available'""", (locker_id,))
